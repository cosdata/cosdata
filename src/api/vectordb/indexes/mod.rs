...
use controller::{
    create_dense_index, create_key_value_index, create_sparse_index, create_tf_idf_index,
    create_usv_index, delete_index,
};
...
pub(crate) fn indexes_module() -> Scope {
    web::scope("/collections/{collection_id}/indexes")
        ...
        .route("/key-value", web::post().to(create_key_value_index))
        .route("/usv", web::post().to(create_usv_index))
