...
use crate::rbac::{RbacUser, Permission};
...
impl AppContext {
    pub fn new(config: Config, args: CosdataArgs) -> Result<Self, WaCustomError> {
        ...
        let rbac_store = Arc::new(RbacStore::new(ain_env.persist.clone())
            .map_err(|e| WaCustomError::DatabaseError(format!("Failed to initialize RBAC store: {}", e)))?);
...
fn setup_initial_admin_user(
    ...
    match store.get_user_by_username(admin_username) {
        Ok(Some(_user)) => {
            log::info!("RBAC admin user '{}' already exists. Verifying role assignment...", admin_username);
            match store.check_permission(admin_username, system_collection_name, Permission::ManagePermissions) {
                Ok(true) => {
                    log::debug!("RBAC admin user '{}' already has necessary permissions on '{}'.", admin_username, system_collection_name);
                    Ok(())
                }
                Ok(false) => {
                     ...
                     match store.assign_role_to_user(admin_username, system_collection_name, admin_role_name) {
                         Ok(()) => {
                         ...
                         Err(e) => {
                             ...
                             Err(WaCustomError::DatabaseError(format!("Failed to assign admin role to existing admin: {}", e)))
                         }
                     }
                }
                Err(e) => {
                     log::error!("Error checking permissions for existing admin user '{}': {}", admin_username, e);
                     Err(WaCustomError::DatabaseError(format!("Failed to check admin permissions: {}", e)))
                }
            }
        }

        Ok(None) => {
            log::info!("RBAC admin user '{}' not found, creating...", admin_username);

            let hash = DoubleSHA256Hash::new(admin_password.as_bytes());
            let password_hash_string: String = hash.0.iter().map(|b| format!("{:02x}", b)).collect();
            ...
            match store.create_user(admin_rbac_user) {
                Ok(created_user) => {
                    log::debug!("Successfully created RBAC admin user '{}' with ID {}.", created_user.username, created_user.user_id);

                    match store.assign_role_to_user(admin_username, system_collection_name, admin_role_name) {
                        Ok(()) => {
                             log::debug!("Successfully assigned admin role to new user '{}'.", admin_username);
                             Ok(())
                        }
                         Err(e) => {
                            log::error!("User created, but failed to assign default admin role to '{}': {}", admin_username, e);
                            Err(WaCustomError::DatabaseError(format!("User created, but failed to assign admin role: {}", e)))
                        }
                    }
                }
                Err(e) => {
                    log::error!("Failed to create RBAC admin user '{}': {}", admin_username, e);
                    Err(WaCustomError::DatabaseError(format!("Failed to create RBAC admin user: {}", e)))
                }
            }
        }
        Err(e) => {
            log::error!("Error checking for RBAC admin user '{}': {}", admin_username, e);
            Err(WaCustomError::DatabaseError(format!("Failed to check RBAC admin user: {}", e)))
        }
    }
}
