...
pub struct USVInputEmbedding(pub InternalId, pub Vec<SparsePair>);

pub struct USVSearchInput(pub Vec<SparsePair>);

pub struct USVSearchOptions {
    pub top_k: Option<usize>,
    pub early_terminate_threshold: Option<f32>,
}

#[derive(Debug, serde::Serialize, serde::Deserialize)]
pub struct USVIndexData {
    pub quantization_bits: u8,
    pub sample_threshold: usize,
}

pub struct USVIndex {
    pub root: USVIndexRoot,
    pub values_upper_bound: RwLock<f32>,
    pub is_configured: AtomicBool,
    pub sampling_data: SamplingData,
    pub vectors: RwLock<Vec<USVInputEmbedding>>,
    pub vectors_collected: AtomicUsize,
    pub sample_threshold: usize,
}

unsafe impl Send for USVIndex {}
unsafe impl Sync for USVIndex {}
...
impl USVIndex {
    pub fn new(
        ...
        let root = USVIndexRoot::new(root_path, quantization_bits)?;
...
impl IndexOps for USVIndex {
    ...
    fn index_embeddings(
        ...
        embeddings
            .into_iter()
            .try_for_each(|USVInputEmbedding(id, pairs)| self.insert(id, pairs, version))?;
        Ok(())
    }
    ...
    fn finalize_sampling(
        ...
        *self.values_upper_bound.write().unwrap() = values_upper_bound;
        ...
        store_usv_values_upper_bound(lmdb, values_upper_bound)?;
        Ok(())
    }
    ...
    fn search_internal(
        ...
        let results = SparseAnnQueryBasic::new(sparse_vec).usv_search(
            &self.root,
