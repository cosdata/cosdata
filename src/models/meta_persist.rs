...
pub fn store_usv_values_upper_bound(lmdb: &MetaDb, bound: f32) -> lmdb::Result<()> {
    let env = lmdb.env.clone();
    let db = lmdb.db;

    let mut txn = env.begin_rw_txn()?;
    let key = key!(m:usv_values_upper_bound);
    let bytes = bound.to_le_bytes();
...
pub fn retrieve_current_version(lmdb: &MetaDb) -> Result<VersionNumber, WaCustomError> {
    let env = lmdb.env.clone();
    let db = lmdb.db;
    let txn = env
        .begin_ro_txn()
    ...
    let hash = VersionNumber::from(u32::from_le_bytes(bytes));

    Ok(hash)
}

/// retrieves the current version of a collection
pub fn retrieve_background_version(lmdb: &MetaDb) -> Result<VersionNumber, WaCustomError> {
    let env = lmdb.env.clone();
    let db = lmdb.db;
    let txn = env
        .begin_ro_txn()
...
pub fn retrieve_usv_values_upper_bound(lmdb: &MetaDb) -> Result<Option<f32>, WaCustomError> {
    let env = lmdb.env.clone();
    let db = lmdb.db;
    let txn = env
        .begin_ro_txn()
