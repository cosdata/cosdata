...
use crate::models::{
    ...
    om_tree_map::{OmQuotientsMap, OmTreeMapNode},
...
impl<T: InlineSerialize> OmTreeMapNode<T> {
    pub fn serialize(
        &self,
        dim_bufman: &BufferManager,
        data_bufmans: &BufferManagerFactory<VersionNumber>,
        ...
        if let Some(offset) = *offset_read_guard {
            ...
            if self.children_last_update_version.load(Ordering::Acquire) >= *version {
                for (idx, child) in self.children.items.iter().enumerate() {
                    let opt_child = unsafe { child.load(Ordering::Relaxed).as_ref() };
                    let Some(child) = opt_child else {
                        dim_bufman
                            .seek_with_cursor(cursor, offset.0 as u64 + 2 + (idx as u64 * 4))?;
                        dim_bufman.update_u32_with_cursor(cursor, u32::MAX)?;
                        continue;
                    };
                    let child_offset =
                        child.serialize(dim_bufman, data_bufmans, cursor, version)?;
                    dim_bufman.seek_with_cursor(cursor, offset.0 as u64 + 2 + (idx as u64 * 4))?;
                    dim_bufman.update_u32_with_cursor(cursor, child_offset)?;
                }
            }

            if self.quotient_last_update_version.load(Ordering::Relaxed) >= *version {
                let quotient_offset =
                    self.quotients
                        .serialize(dim_bufman, data_bufmans, cursor, version)?;
                dim_bufman.seek_with_cursor(cursor, offset.0 as u64 + 34)?;
                dim_bufman.update_u32_with_cursor(cursor, quotient_offset)?;
            }
        ...
        if let Some(offset) = *offset_write_guard {
            ...
            if self.quotient_last_update_version.load(Ordering::Acquire) >= *version {
                let quotient_offset =
                    self.quotients
                        .serialize(dim_bufman, data_bufmans, cursor, version)?;
                dim_bufman.seek_with_cursor(cursor, offset.0 as u64 + 34)?;
        ...
        for child in &self.children.items {
            ...
            let offset = child.serialize(dim_bufman, data_bufmans, cursor, version)?;
            buf.extend(offset.to_le_bytes());
        }
        let quotient_offset =
            self.quotients
    ...
    pub fn deserialize(
        ...
        let quotients_offset = dim_bufman.read_u32_with_cursor(cursor)?;
        let quotients = OmQuotientsMap::deserialize(
            dim_bufman,
            data_bufmans,
            FileOffset(quotients_offset),
            version,
