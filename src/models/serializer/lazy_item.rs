    ...
    let offset = if offset == u32::MAX {
        ...
        let last_chunk = loop {
            for j in 0..CHUNK_SIZE {
                bufman.seek_with_cursor(cursor, current_chunk as u64 + (j as u64 * 10))?;
                let item_offset = bufman.read_u32_with_cursor(cursor)?;
                if item_offset == u32::MAX {
                    continue;
                }
                if let Some(item) = items.get(i) {
                    bufman.seek_with_cursor(cursor, item_offset as u64)?;
        ...
        if items.len() > i {
            ...
            let current_pos = bufman.cursor_position(cursor)?;
            bufman.seek_with_cursor(cursor, last_chunk as u64 + (CHUNK_SIZE as u64 * 10))?;
            bufman.update_u32_with_cursor(cursor, current_pos as u32)?;
            bufman.seek_with_cursor(cursor, current_pos)?;
...
fn lazy_item_serialize_impl<T: Cacheable + CustomSerialize>(
    ...
    if serialized_flag {
        ...
        bufman.seek_with_cursor(cursor, versions_placeholder)?;
        bufman.update_u32_with_cursor(cursor, offset)?;
        bufman.seek_with_cursor(cursor, current_position)?;
    } else {
        ...
        bufman.seek_with_cursor(cursor, versions_placeholder)?;
        bufman.update_u32_with_cursor(cursor, versions_offset)?;

        bufman.seek_with_cursor(cursor, end_offset)?;
    }
