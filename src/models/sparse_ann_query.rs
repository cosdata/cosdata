...
#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct SparseAnnResultWithMatches {
    pub vector_id: u32,
    pub similarity: u32,
...
impl SparseAnnQueryBasic {
    ...
    pub fn search_with_matches(
        ...
        reranking_factor: usize,
        k: Option<usize>,
    ) -> Result<Vec<SparseAnnResultWithMatches>, BufIoError> {
        let mut dot_products = FxHashMap::default();
        // same as `1` quantized
        let one_quantized = ((1u32 << quantization_bits) - 1) as u8;
        let early_terminate_value = ((1u32 << quantization_bits) as f32 * early_terminate_threshold)
                ...
                for key in (0..=one_quantized).rev() {
                    unsafe { &*node.data }
                        ...
                        .with_value(&key, |vec| {
                            for vec_id in vec.iter() {
                                let dot_product =
                                    dot_products.entry(vec_id).or_insert((0u32, Vec::new()));
                                dot_product.0 += quantized_query_value * key as u32;
                                dot_product
                                    .1
                                    .push((dim_index, quantized_query_value * key as u32));
                            }
                        });
                }
            } else {
        ...
        // Convert the heap to a vector and reverse it to get descending order
        let mut results: Vec<SparseAnnResultWithMatches> = dot_products
            .into_iter()
            .map(
                |(vector_id, (similarity, matches))| SparseAnnResultWithMatches {
                    vector_id,
                    similarity,
                    matches,
                },
