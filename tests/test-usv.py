...
DATASET_FILE = "datasets/usv_dataset/vectors.pkl"
QUERY_VECTORS_FILE = "datasets/usv_dataset/query.pkl"
BRUTE_FORCE_RESULTS_FILE = "datasets/usv_dataset/brute_force_results.pkl"
...
def create_db(name, description=None):
...
def create_explicit_index(name):
    url = f"{client.base_url}/collections/{name}/indexes/usv"
    data = {"name": name, "quantization": 64, "sample_threshold": 1000}
    response = requests.post(url, headers=client._get_headers(), data=json.dumps(data), verify=client.verify_ssl)
    if response.status_code not in [200, 201]:
        raise Exception(f"Failed to create USV index: {response.text}")
...
def upsert_in_transaction(name, txn_id, vectors):
    """
...
def search_sparse_vector(name, query_terms, top_k=10, early_terminate_threshold=0.0):
    """Search sparse vector"""
    # query_terms should already be in the correct format from format_for_server_query
    url = f"{client.base_url}/collections/{name}/search/usv"
    data = {
...
def batch_ann_search(name, batch, top_k=10, early_terminate_threshold=0.0):
    """Batch sparse search using cosdata client"""
    try:
        url = f"{client.base_url}/collections/{name}/search/batch-usv"
        data = {
...
def main():
    vector_db_name = "usv_test_db"
    num_vectors = 1_000_000
    dimension = 20_000_000
    ...
    Path("datasets/usv_dataset").mkdir(parents=True, exist_ok=True)
    ...
    if insert_vectors:
        ...
        txn_id = create_transaction(vector_db_name)
        with ThreadPoolExecutor(max_workers=32) as executor:
            futures = []
            for batch_start in range(0, num_vectors, batch_size):
                batch = vectors[batch_start : batch_start + batch_size]
                futures.append(
                    executor.submit(upsert_in_transaction, vector_db_name, txn_id, batch)
                )
        ...
        commit_transaction(vector_db_name, txn_id)
        
        end = time.time()
        insertion_time = end - start
        print(f"Insertion time: {insertion_time:.2f} seconds")
        print(f"Transaction committed with ID: {txn_id}")
    ...
    with open("datasets/usv_dataset/vector_evaluation_results.pkl", "wb") as f:
        pickle.dump(detailed_results, f)

    print(
        "Detailed results saved to datasets/usv_dataset/vector_evaluation_results.pkl"
    )
