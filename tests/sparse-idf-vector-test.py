...
def custom_rng(min_val: int, max_val: int, probs, num_bins, step) -> int:
    ...
    return np.random.randint(
        min_val + chosen_bin * step, min_val + (chosen_bin + 1) * step
    )
...
def select_query_vectors(vectors, num_queries):
    ...
    for idx in query_indices:
        ...
        indices, raw_term_frequencies, values, brute_force_values = (
            zip(*top_terms) if top_terms else ([], [], [], [])
        )
...
def run_rps_tests(
    ...
    print(
        f"Using {len(rps_test_vectors)} different test vectors for RPS testing (top 10 terms only)"
    )
    ...
    for vector in rps_test_vectors:
        ...
        term_data = list(
            zip(vector["indices"], vector["raw_term_frequencies"], vector["values"])
        )
        ...
        truncated_vectors.append(
            {"id": vector["id"], "indices": list(indices), "values": list(values)}
        )
